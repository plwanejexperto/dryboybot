...
WEBHOOK_URL = f"https://plwanejexperto.pythonanywhere.com{WEBHOOK_PATH}"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
	...
	latest_station_map = {
		station['id'].strip().upper(): station['name']
		for station in latest_rainfall_data['data']['stations']
	}
	...
	await update.message.reply_text(
		"Welcome! Please choose a location.\n"
		"You may click this link to view the map of the locations:\n"
		"https://www.google.com/maps/d/u/0/edit?mid=1EXDQiXeSiNkbfT6LaXIpT5nv-2n1ErQ&usp=sharing",
...
async def handle_station_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
	...
	try:
		...
		await query.edit_message_text("Sorry, failed to fetch weather data. Please try again later.")
		return
	...
	if choice == "SHOW_ALL":
		...
		for sid in location_info["stations"]:
			...
			print(f"Station {sid_norm} rainfall: {value}")
			if value > 0:
		...
		for sid in location_info["stations"]:
			...
			print(f"Station {sid_norm} temperature: {temp_value}")
			if temp_value is not None:
				name = station_map.get(sid_norm, sid_norm)
				temp_lines.append(f"{name} ({sid_norm}): {temp_value}°C")
	
		reply_parts = [f"📍 {choice}", f"⏰ {format_timestamp(readings['timestamp'])}"]
		if forecast_text:
...
application.add_handler(CallbackQueryHandler(handle_station_choice))
